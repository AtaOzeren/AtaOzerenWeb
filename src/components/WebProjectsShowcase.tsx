...
// Animation configuration constants for the showcase
...
const WebProjectsShowcase: Component<WebProjectsShowcaseProps> = (props) => {
    const { t } = useI18n();
    ...
    const initializeProjectAnimation = () => {
        const projectItems = document.querySelectorAll('.showcase-project-item');
        ...
        gsap.fromTo(projectItems, {
            ...
            opacity: 1,
            x: 0,
            duration: SHOWCASE_ANIMATIONS.projectItems.duration,
            stagger: SHOWCASE_ANIMATIONS.projectItems.stagger,
            ease: SHOWCASE_ANIMATIONS.projectItems.ease,
            delay: SHOWCASE_ANIMATIONS.projectItems.delay
        });
    };
    ...
    const startSyconxGlowAnimation = (): void => {
        const syconxLinks = document.querySelectorAll('.showcase-syconx-link');

        syconxLinks.forEach(link => {
            gsap.to(link, {
                textShadow: SHOWCASE_ANIMATIONS.syconxGlow.glowShadow,
                duration: SHOWCASE_ANIMATIONS.syconxGlow.duration,
                ease: SHOWCASE_ANIMATIONS.syconxGlow.ease,
                yoyo: true,
                repeat: 1,
                onComplete: () => {
                    setTimeout(startSyconxGlowAnimation, SHOWCASE_ANIMATIONS.syconxGlow.interval);
                }
            });
        });
    };
    ...
    const setupSyconxHoverAnimations = () => {
        ...
        syconxLinks.forEach(link => {
            link.addEventListener('mouseenter', () => {
                ...
                gsap.to(link, {
                    duration: SHOWCASE_ANIMATIONS.projectHover.duration,
                    scale: 1.1,
                    textShadow: SHOWCASE_ANIMATIONS.syconxGlow.hoverShadow,
                    ease: SHOWCASE_ANIMATIONS.projectHover.ease
                });
            });
    ...
    const handleMouseEnter = (projectId: string, event: MouseEvent) => {
        ...
        gsap.to(target, {
            duration: SHOWCASE_ANIMATIONS.projectHover.duration,
            x: SHOWCASE_ANIMATIONS.projectHover.translation,
            scale: SHOWCASE_ANIMATIONS.projectHover.scale,
            ease: SHOWCASE_ANIMATIONS.projectHover.ease
        });
    ...
    const animateProjectImage = () => {
        const imageElement = document.querySelector('.showcase-project-image');
    ...
    const animateFlipIcon = (show: boolean) => {
        const iconElement = document.querySelector('.showcase-flip-icon');
        ...
        if (show) {
            gsap.fromTo(iconElement, {
                ...
                scale: 1,
                rotation: 0,
                duration: SHOWCASE_ANIMATIONS.flipIcon.duration,
                ease: SHOWCASE_ANIMATIONS.flipIcon.ease
            });
        } else {
            gsap.to(iconElement, {
                opacity: 0,
    ...
    const handleFlip = () => {
        ...
        const cardElement = document.querySelector('.showcase-project-card');
        if (cardElement) {
            gsap.to(cardElement, {
                rotationY: isCurrentlyFlipped ? 0 : 180,
                duration: SHOWCASE_ANIMATIONS.flipCard.duration,
                ease: SHOWCASE_ANIMATIONS.flipCard.ease,
                transformOrigin: "center center"
            });
        }
    };
    ...
    return (
        <div class={props.containerClass || "flex w-full max-w-6xl"}>
            {/* Left Side - Project List */}
            <div class="flex-1 flex flex-col justify-center space-y-8">
                {projects.map((project) => (
                    <div
                        class="showcase-project-item cursor-pointer"
            ...
            <div class="flex-1 flex items-center justify-center pl-16">
                {hoveredProject() && (
                    <div
                        class="showcase-project-image relative w-[500px] h-[500px] perspective-1000"
                        onMouseEnter={handleImageHover}
                        onMouseLeave={handleImageLeave}
                    >
                        {/* Flip Card Container */}
                        <div
                            class="showcase-project-card relative w-full h-full preserve-3d cursor-pointer"
                        ...
                        >
                            ...
                            <div class="absolute inset-0 w-full h-full backface-hidden rounded-lg shadow-2xl overflow-hidden">
                                ...
                                {/* Flip Icon */}
                                {showIcon() && (
                                    <div class="showcase-flip-icon absolute top-4 right-4 w-12 h-12 bg-black/80 backdrop-blur-md rounded-full flex items-center justify-center border-2 border-gray-300/60 shadow-xl hover:bg-black/90 hover:scale-110 transition-all duration-300">
                                        <svg
                                            ...
                                            <div class="text-center">
                                                ...
                                                <p class="text-sm text-gray-400 uppercase tracking-wider mb-6">
                                                    <a
                                                        ...
                                                        class="showcase-syconx-link text-white hover:text-gray-200 transition-all duration-300 no-underline font-medium"
...
export default WebProjectsShowcase;

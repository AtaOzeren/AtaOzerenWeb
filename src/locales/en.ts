import type { Locale } from './tr';

export const en: Locale = {
    // Navbar
    nav: {
        home: "HOME",
        about: "ABOUT",
        projects: "PROJECTS",
        contact: "Contact",
    },

    // Hero Section
    hero: {
        name: "Ata Özeren",
        title: "Web Developer & Software Engineer",
        description: "I develop user-friendly and performant applications using modern web technologies.",
        cta: "View My Projects",
        contact: "Get In Touch",
    },

    // About Section
    about: {
        title: "About Me",
        subtitle: "Passionate developer",
        description: "I'm an experienced software developer in web development, creating effective solutions using modern technologies.",
        experience: "Experience",
        skills: "Skills",
        years: "years",
        content: {
            paragraph1: "I am Ata Özeren, born in 2000 in Izmir, Turkey. I am a software developer with experience in full-stack web and mobile application development. I specialize in modern programming languages and frameworks such as JavaScript, React, React Native, SQL, and Python.",
            paragraph2: "I have gained experience by working on both domestic and international projects. I am currently employed as a Frontend Developer at SyconX, where I focus on building scalable and user-friendly interfaces. Previously, I worked as a remote Full-Stack Developer at Martsys Software on a project-based basis, and as a developer at YeliApp, where I contributed to in-office project development.",
            paragraph3: "I am pursuing a Bachelor's degree in Management Information Systems at Anadolu University. Additionally, I hold a Bachelor's degree in Physics and an Associate degree in Computer Programming from Atatürk University. I have also completed the CS50 Web Programming course from Harvard University and received Python training at Middle East Technical University (METU)."
        },
        technologies: "Technologies I Use",
        github: {
            contributions: "GitHub Contributions",
            viewProfile: "View GitHub Profile"
        }
    },

    // Projects Section
    projects: {
        title: "My Projects",
        subtitle: "Recent work",
        viewProject: "View Project",
        sourceCode: "Source Code",
        appShowcase: "App Showcase",
        appShowcaseDesc: "Mobile and web applications I've developed",
        visitSite: "Visit Site",
        developedBy: "developed by",
        scrollToApps: "Go to Apps",
        continueReading: "Continue Reading",
        noProjects: "No projects yet",
        noProjectsDesc: "Projects in this category will be added soon.",
        categories: {
            frontend: "Frontend",
            backend: "Backend",
            mobile: "Mobile",
            crm: "CRM",
            mcp: "MCP",
            active: "Active Projects"
        },
        projectTypes: {
            corporate: "Corporate",
            ecommerce: "E-Commerce",
            crm: "CRM",
            mcp: "MCP Server"
        },
        // Project names
        erdemli: "Erdemli Architecture",
        ege: "Ege Architecture",
        eje: "Eje Studio",
        delta: "Delta Nursery",
        sanercrm: "Saner CRM",
        saner: "Saner Residences",
        savuncell: "Savuncell",
        turanlar: "Turanlar Holding",
        ykt: "Ykt Global",
        meetlyme: "MeetlyMe",
        mehves: "Mehves Giyim",
        perlei: "Perlei",
        chargecrafter: "Charge Crafter",
        descriptions: {
            erdemli: "Corporate website for modern architecture services",
            ege: "Professional architecture and design services",
            eje: "Creative design studio website",
            delta: "Sapling and landscaping services platform",
            sanercrm: "Customer relationship management system",
            saner: "Real estate and housing projects site",
            savuncell: "Technology and innovation focused platform",
            turanlar: "Holding company corporate website",
            ykt: "Global trade and logistics platform",
            meetlyme: "This website is an online planning and collaboration tool that helps users easily schedule meetings for business meetings and events, share their availability with a single link, and manage projects/teams from a single place.",
            mehves: "An online showcase promoting the products of Izmir-based retailer and wholesaler Mehveş Giyim, providing store and product information to customers.",
            perlei: "An online showcase promoting the products of Izmir-based retailer and wholesaler Perlie Giyim, providing store and product information to customers.",
            chargecrafter: "A comprehensive management tool that consolidates invoicing, sending, and customer financial management into a single platform for businesses. It aims to increase operational efficiency by simplifying processes such as payment tracking, automated invoicing, and streamlining business workflows.",
            meetlyme_mcp: "Node.js-based AI-powered MCP server handling site-specific operations without a database.",
            trivia_game_backend: "Trivia game backend project developed using Cloudflare D1 database and TypeScript.",
            trivia_game: "Mobile Trivia game developed using React Native, Tailwind CSS, and Expo Go.",
            popy_log: "Mobile logging and tracking app developed using React Native, Tailwind CSS, and Expo Go.",
            pendingPublication: "Ready, waiting for publication."
        }
    },    // Contact Section
    contact: {
        title: "Contact",
        subtitle: "Get in touch with me",
        name: "Your Name",
        email: "Email",
        phone: "Phone",
        message: "Your Message",
        send: "Send",
        sending: "Sending...",
        success: "Your message has been sent successfully!",
        error: "An error occurred. Please try again.",
        clickToEmail: "Click to send email",
        clickToCall: "Click to call",
        location: "Turkey",
    },

    // Footer
    footer: {
        rights: "All rights reserved.",
        madeWith: "Made with",
    },

    // Common
    common: {
        loading: "Loading...",
        error: "Error",
        success: "Success",
        close: "Close",
        open: "Open",
        scrollDown: "Scroll Down",
        scrollUp: "Scroll Up",
    },
    // Not Found
    notFound: {
        doomMessage: "We couldn't find the page you were looking for, but you can play DOOM here instead."
    },
};
